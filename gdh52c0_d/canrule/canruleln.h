...
class CanRuleLn
{
    ...
    u32 m_ModuleAddrlist;// 模块地址列表（记录所有模块地址）
...
u8 DuleWithData(PCanBuf pCanbuf,DATABASE *pdatabase);	// 处理接收的CAN数据
void ModuleOnLineDetect(DATABASE *pdatabase);
bool PollingModuleCmd(PCanBuf pCanbuf);// 轮询发送模块命令
bool SendCmd(PCanBuf pCanbuf,DATABASE *pdatabase);	
void Init(u8 prol);		
void OnTick(PCanBuf pCanbuf,DATABASE *pdatabase);		// 定时任务（周期性执行）
