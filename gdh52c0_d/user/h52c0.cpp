...
u16 h52c0::SaveRecordInfo(u16 BaseAddr,void * pRecord,u16 ReLen,u8 ch)
{
	...
	if(ch==1)
	{
		// 1. 从E2PROM的BaseAddr地址读取2字节数据，存入flag（记录标记）
	 ...
	 //初始化用户配电记录长度信息			
        ...
        for(u8 i=0;i<4;i++)	
			 {				
				      ...
				      *((u16 *)&gBattPara[i].battEnergy) = bkp_data_read((bkp_data_register_enum)(2*i+1));
				     *((u16 *)&gBattPara[i].battEnergy+1) = bkp_data_read((bkp_data_register_enum)(2*i+2));
				      ...
				      for(u8 i=0;i<TOTAL_USER+8;i++)
							{
								  ...
								  gSwitchPara[i].st_downV=4400;
								  gSwitchPara[i].st_recoverV=4700;
								  gSwitchPara[i].st_downT=180;
								  gSwitchPara[i].st_downMode=1;
								  gSwitchPara[i].st_downEnery=0;
