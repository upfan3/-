...
static __align(64) uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((at(0XC01F4000)));     /* 外部SDRAM内存池,前面2MB用做RGB LCD屏的显存(1280*800*2) */
...
static MT_TYPE mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((at(0XC01F4000 + MEM3_MAX_SIZE)));  /* 外部SDRAM内存池MAP */
#else      /* 使用AC6编译器时 */
/* 内存池(64字节对齐) */
...
static __ALIGNED(64) uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((section(".bss.ARM.__at_0XC01F4000")));      /* 外部SDRAM内存池,前面2MB用做RGB LCD屏的显存(1280*800*2) */ 

/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                                                              /* 内部SRAM内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0x1000F000")));         /* 内部CCM内存池MAP */
static MT_TYPE mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0XC1E30000")));         /* 外部SDRAM内存池MAP */
#endif
