...
int oid_greater_than(_oid *oid_a, _oid *oid_b)
{
    ...
    for (i = 0; i < length; i++)
    {
        if (oid_b->val[i] > oid_a->val[i])
            return 1;
        if (oid_b->val[i] < oid_a->val[i])
            return 0;
    }

    if (oid_b->len > oid_a->len)
        return 1;
    return 0;
}
...
int mib_add_var_binding(unsigned char *data, int data_len, _oid *oid, void *snmp_data, int type)
{
    ...
    case END_OF_MIB_VIEW:
        p1 += asn_encode_end_of_mib_view(p1);
        break;
    case NO_SUCH_OBJECT:
        p1 += asn_encode_no_such_object(p1);
        break;
    case NO_SUCH_INSTANCE:
        p1 += asn_encode_no_such_instance(p1);
        break;
    }
...
int mib_process_oid(unsigned char *data, _oid *oid, int request, int data_type, void *data_in, int data_in_len, int *error)
{
    ...
    if ((request == GET_NEXT_REQUEST_PDU) && ((ret == ASN_UNKNOWN) || (ret == NO_SUCH_OBJECT)))
    {
