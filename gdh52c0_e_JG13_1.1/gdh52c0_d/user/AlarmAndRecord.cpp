...
u8 battbroadflag=0; // bit7:flash, bit6:通信状态,bit1:熔丝2,bit0:熔丝1
u8 battbroadfuse[2]={0,0};
...
u8 gledtimer;
u8 ation[]={ NO,UNNOMAL,UNNOMAL,BROKEN,BROKEN,BROKEN,BREAK,BREAK,BREAK,BREAK,BREAK,BREAK};
...
void removeModule(u8 nb,u8 type)//从1号开始
{
	...
	{
		dcdistributioncommbroken&=(~((uint64_t)1<<(nb-1)));
		SwitchOnlineCount[nb-1]=0;
	}
...
{ 
    ...
    //
				...
				if(SwitchOnlineCount[j]>0)
				{
							 moduleNum++;
								...
								if((gDCdistribution.pst_status[j]&0x01)==1)//&&(gSwitchPara[i].st_downV>*((u16 *)&gpSysData[DCVOLTAGE])))
								{
									 if((dcdistributionflag&(((int64_t)1<<j)))==0)
									 {
										 ...
										 if(gSwitchPara[j].st_userGroud!=0x7B)
										 {
													 ...
													 addAlarm(USER_DOWN,moduleNum,DOWNELE); 
										 }
										 else
										 {
												 
												 addAlarm(BATTDOWN_ALARM,moduleNum,DOWNELE); 
										 }
								...
								else
								{
									 ...
									 if((dcdistributionflag&(((int64_t)1<<j)))!=0)
									 {
										 ...
										 if(gSwitchPara[j].st_userGroud!=0x7B)
										 {
												 ...
												 minusAlarm(USER_DOWN,moduleNum,DOWNELE); 
										 }
											else
										 {
													
												 minusAlarm(BATTDOWN_ALARM,moduleNum,DOWNELE); 
										 }
								...
								if((gDCdistribution.pst_status[j]&0x40)==0x40)//&&(gSwitchPara[i].st_downV<=*((u16 *)&gpSysData[DCVOLTAGE])))
								{
												 if((dcdistributionflag2&(((int64_t)1<<j)))==0)
												 {
													 ...
													 if(gSwitchPara[j].st_userGroud!=0x7B)
													 {
																...
																addAlarm(USER_DOWN,moduleNum,BREAK); 				 
														 
													 }
													 else
													 {
															addAlarm(BATTFUSR_BREAK,moduleNum,BREAK);  
								...
								else
								{
												 ...
												 if((dcdistributionflag2&(((int64_t)1<<j)))!=0)
												 {
													 ...
													 if(gSwitchPara[j].st_userGroud!=0x7B)
													 {
															 minusAlarm(USER_DOWN,moduleNum,BREAK);  
															 
													 }
														else
													 {
															 minusAlarm(BATTFUSR_BREAK,moduleNum,BREAK);  
							...
							//过载告警
								if((gDCdistribution.pst_status[j]&0x08)==0x08)
								{
										 ...
										 {
											 addAlarm(USER_DOWN,moduleNum,OVER_LOAD); 
								...
								else
								{
										 if((dcdistributionOverLoad&(((int64_t)1<<j)))!=0)
										 {
													 minusAlarm(USER_DOWN,moduleNum,OVER_LOAD); 
													dcdistributionOverLoad&=~((int64_t)1<<j);
										 }
								...
								if((gDCdistribution.pst_status[j]&0x80)==0x80)
								{
										 ...
										 {
											 addAlarm(USER_DOWN,moduleNum,BROKEN); 
								...
								else
								{
										 if((dcdistributionbroken&(((int64_t)1<<j)))!=0)
										 {
													 minusAlarm(USER_DOWN,moduleNum,BROKEN); 
													dcdistributionbroken&=~((int64_t)1<<j);
										 }
							...
							if(SwitchOnlineCount[j]==1)
							{
									if(((dcdistributioncommbroken)&((uint64_t)1<<j))==0)
									{
											...
											addAlarm(USER_DOWN,j+1,COMM_BROKEN); 
									}
							}
							else if(SwitchOnlineCount[j]>1)
							{
								if(((dcdistributioncommbroken)&((uint64_t)1<<j))!=0)
								{
										...
										minusAlarm(USER_DOWN,j+1,COMM_BROKEN); 
								}
							}
		...
		{//电池下电板
			      if( (*((u16 *)&gpSysData[ACID_BATT_STATUS])&0x0100)!=0)//告警
						{
							   if((battbroadflag&0x01)==0)
								 {
									  addAlarm(BATTFUSR_BREAK,1,BREAK);  
			      ...
			      else
						{
							  if((battbroadflag&0x01)!=0)
								 {
									  minusAlarm(BATTFUSR_BREAK,1,BREAK);  
									 battbroadfuse[0]=0;
								 }
								 battbroadflag&=0xFE;
						}
						
						 if( (*((u16 *)&gpSysData[ACID_BATT_STATUS])&0x0200)!=0)//告警
						{
							   if((battbroadflag&0x02)==0)
								 {
			      ...
			      else
						{
						  ...
						  if( (*((u16 *)&gpSysData[ACID_BATT_STATUS])&0x0080)!=0)//告警
						  {
								 if((battbroadflag&0x80)==0)
								 {
									   addAlarm(IN_DV,1,FLASH_BROKEN);  
								 }
							...
							else
							{
								 if((battbroadflag&0x80)!=0)
								 {
									   minusAlarm(IN_DV,1,FLASH_BROKEN);  
								 }
						...
						{
							  if((battbroadflag&0x40)==0)
								 {
									    addAlarm(IN_DV,1,COMM_BROKEN);  
									 
									    *((s16 *)&gpSysData[BATT_CURR5])=0;
									    *((s16 *)&gpSysData[BATT_CURR6])=0;
						...
						else
						{
							  	 if((battbroadflag&0x40)!=0)
								 {
									   minusAlarm(IN_DV,1,COMM_BROKEN);  
								 }
