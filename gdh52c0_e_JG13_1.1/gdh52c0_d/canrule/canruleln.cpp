...
u8 acOnlineTimer[3]={0xFF,0xFF,0xFF};
								        ...
								        if(pdatabase!=NULL)
												{
												     ...
												     u16 *pvdat=((u16 *)pdatabase->GetDataAddr(CURRENT_AC));
													   u32 tmpdat=bswap_32(plndat->st_cmdData);
													      // tmpdat=tmpdat;
													   if((s32)tmpdat<25000)//单模块输入交流电流不超过25A
													  pvdat[testaddr]=(u16)tmpdat;
						  ...
						  //if(cmd==SET_OUT_VOLTAGE)
						   if(m_cmd==SET_OUT_VOLTAGE)
							{												
								...
								if((m_floatorequal==(u8)FLOAT_MOD)||(m_floatorequal==(u8)EQUAL_MOD))
								{
									if(prol==DCDC)
						      { 
