...
void norflash_init(void)
{
    ...
    rcu_periph_clock_enable(RCU_GPIOF);    /* NORFLASH CS脚 时钟使能 */
    ...
    spi4.init();                        /* 初始化SPI4 */
    spi4.set_speed(SPI_SPEED_4);        /* SPI4 切换到高速状态 30Mhz */
    ...
    if (g_norflash_type == W25Q256)     /* SPI FLASH为W25Q256, 必须使能4字节地址模式 */
    {
        ...
        if ((temp & 0X01) == 0)         /* 如果不是4字节地址模式,则进入4字节地址模式 */
        {
            ...
            NORFLASH_CS(0);
            spi4.read_write_byte(FLASH_Enable4ByteAddr);    /* 使能4字节地址指令 */
            NORFLASH_CS(1);
        }
    }
...
{
    NORFLASH_CS(0);
    spi4.read_write_byte(FLASH_WriteEnable);   /* 发送写使能 */
    NORFLASH_CS(1);
}
...
static void norflash_send_address(uint32_t address)
{
    ...
    {
        spi4.read_write_byte((uint8_t)((address)>>24)); /* 发送 bit31 ~ bit24 地址 */
    } 
    spi4.read_write_byte((uint8_t)((address)>>16));     /* 发送 bit23 ~ bit16 地址 */
    spi4.read_write_byte((uint8_t)((address)>>8));      /* 发送 bit15 ~ bit8  地址 */
    spi4.read_write_byte((uint8_t)address);             /* 发送 bit7  ~ bit0  地址 */
}
...
uint8_t norflash_read_sr(uint8_t regno)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(command);      /* 发送读寄存器命令 */
    spi4.read_write_byte(0Xff,&byte);  /* 读取一个字节 */
    NORFLASH_CS(1);
...
void norflash_write_sr(uint8_t regno, uint8_t sr)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(command);  /* 发送写状态寄存器命令 */
    spi4.read_write_byte(sr);       /* 写入一个字节 */
    NORFLASH_CS(1);
}
...
uint16_t norflash_read_id(void)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(FLASH_ManufactDeviceID);   /* 发送读 ID 命令 */
    spi4.read_write_byte(0);    /* 写入一个字节 */
    spi4.read_write_byte(0);
    spi4.read_write_byte(0);
	
	   uint8_t tmph=0,tmpl=0;
      spi4.read_write_byte(0xFF,&tmph) ;     /* 读取高8位字节 */
      spi4.read_write_byte(0xFF,&tmpl);         /* 读取低8位字节 */
	  deviceid = (tmph<<8)|tmpl;
    NORFLASH_CS(1);
...
void norflash_read(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(FLASH_ReadData);       /* 发送读取命令 */
    norflash_send_address(addr);                /* 发送地址 */
    
    for(i=0;i<datalen;i++)
    {
        spi4.read_write_byte(0XFF,&pbuf[i]);   /* 循环读取 */
    }
...
static void norflash_write_page(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(FLASH_PageProgram);    /* 发送写页命令 */
    norflash_send_address(addr);                /* 发送地址 */

    for(i=0;i<datalen;i++)
    {
        spi4.read_write_byte(pbuf[i]);          /* 循环写 */
    }
...
void norflash_erase_chip(void)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(FLASH_ChipErase);  /* 发送芯片擦除命令 */ 
    NORFLASH_CS(1);
...
void norflash_erase_sector(uint32_t saddr)
{
    ...
    NORFLASH_CS(0);
    spi4.read_write_byte(FLASH_SectorErase);    /* 发送扇区擦除指令 */
