...
void WIZ_SPI_Init(void)
{
    ...
    spi_init(SPI2, &spi_init_struct);
		...
		while ( spi_i2s_flag_get(SPI2, SPI_FLAG_TBE)==RESET){;}//等待发送寄存器为空
   	//spi_enable(SPI2);
...
uint8_t SPI2_SendByte(uint8_t byte)
{
	   while ( spi_i2s_flag_get(SPI2, SPI_FLAG_TBE)==RESET);//等待发送寄存器为空
		 ...
		 while ( spi_i2s_flag_get(SPI2, SPI_FLAG_RBNE)==RESET);//等待送发寄存器不为空
...
void SPI_DMA_WRITE(uint8_t* Addref, uint8_t* pTxBuf, uint16_t tx_len)
{
	...
	for(i=0; i<tx_len; i++)
	{
	  ...
	  dma_data_parameter.number       = (uint16_t)(tx_len + 3);//数据缓存大小
	  dma_data_parameter.periph_addr  = (uint32_t)&SPI_DATA(SPI2);//外设地址
    dma_data_parameter.periph_inc   = DMA_PERIPH_INCREASE_DISABLE;//不执行外设地址增量模式
    dma_data_parameter.memory_inc   = DMA_MEMORY_INCREASE_ENABLE;//存储器地址递增
    dma_data_parameter.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;//外设数据宽度8位
    dma_data_parameter.memory_width = DMA_MEMORY_WIDTH_8BIT; //存储器数据宽度8位 
 
    dma_circulation_disable(DMA1, DMA_CH0);//不执行循环操作
		dma_circulation_disable(DMA1, DMA_CH1);//不执行循环操作
    dma_data_parameter.priority     = DMA_PRIORITY_HIGH;//优先级--高
    
		dma_memory_to_memory_disable(DMA1, DMA_CH0);//存储器到存储器失能
		dma_memory_to_memory_disable(DMA1, DMA_CH1);//存储器到存储器失能
  ...
  /* Configure TX DMA */
	...
	dma_data_parameter.direction    = DMA_MEMORY_TO_PERIPHERAL;//数据传输方向存储器到外设（从存储器读）
	dma_data_parameter.memory_addr  = (uint32_t )SPI2_DMA_TxBuff ;//数据存储地址
	dma_init(DMA1, DMA_CH1, &dma_data_parameter);
	
  /* Configure RX DMA */

  dma_data_parameter.direction    = DMA_PERIPHERAL_TO_MEMORY;;//数据传输方向外设到存储器（从外设读）
	dma_data_parameter.memory_addr  = (uint32_t )SPI2_DMA_TxBuff ;//数据存储地址
  ...
  /* Waiting the end of Data transfer */
    while(dma_flag_get(DMA1, DMA_CH1, DMA_FLAG_FTF)==RESET);//等待数据传输完成
...
//配置DMA	
    ...
    dma_data_parameter.number       = (uint16_t)(rx_len + 3);//数据缓存大小
	  dma_data_parameter.periph_addr  = (uint32_t)&SPI_DATA(SPI2);//外设地址
    dma_data_parameter.periph_inc   = DMA_PERIPH_INCREASE_DISABLE;//不执行外设地址增量模式
    dma_data_parameter.memory_inc   = DMA_MEMORY_INCREASE_ENABLE;//存储器地址递增
    dma_data_parameter.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;//外设数据宽度8位
  ...
  /* Configure TX DMA */
  dma_data_parameter.direction    = DMA_MEMORY_TO_PERIPHERAL;;//数据传输方向外设到存储器（从外设读）
	dma_data_parameter.memory_addr  = (uint32_t )SPI2_DMA_TxBuff ;//数据存储地址
	dma_init(DMA1, DMA_CH1, &dma_data_parameter);
