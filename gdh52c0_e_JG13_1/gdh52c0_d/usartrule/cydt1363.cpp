		...
		{//铁塔白盒化指令
			...
			if(m_addr==monitor_addr)//检查通信地址 若通信地址不正确时不返回数据
			{
							...
							if(m_cid2==0x41)    //模拟量指令
							{
									 ...
									 {
											 set4341();//光伏模块
//                                                                 }else if(m_cid1==0xE1){ setE141();//系统
									 }
									 else if(m_cid1==0xE2)
									 { 
										 setE241();//配电分路
	                ...
	                if(command_group==0xff)
	                  ...
	                  SetFourByteData((float)(*((s16 *)&gpSysData[USER_CURR]))/10); //负载总电流
										...
										if(gbattGroud==0)//非智能开关，电池分路
										 {
                         SetOneByteData(1);//电池路数
//											   SetFourByteData((float)(*((s16 *)&gpSysData[BATT_CURR5]))*sysPara[BATT5_EN]/10); //电池1电流(铅酸单元)
//				                 SetFourByteData((float)(*((s16 *)&gpSysData[BATT_CURR6]))*sysPara[BATT6_EN]/10); //电池2电流(铅酸单元)
...
//					               SetFourByteData((float)(*((s16 *)&gpSysData[BATT_CURR1]))/10); //电池1电流（传感器1 AD0）
//				                 SetFourByteData((float)(*((s16 *)&gpSysData[BATT_CURR2]))/10); //电池2电流 (传感器1 AD1)
										 ...
										 float tmpI=((float)(*((s16 *)&gpSysData[LOADCURR]))/10-(float)(*((s16 *)&gpSysData[TOTAL_BATTI]))/10-(float)totalUserI/10);
										 if(tmpI<0)tmpI=0;
					           ...
					           // SetOneByteData(4+battGroud);//铁塔自定义字段数量 a=3+1+m m=1
										 SetOneByteData(4+1);//铁塔自定义字段数量 a=3+1+m m=1
		...
		///////////////////////////////租户部分的遥测内容//////////////////////////////////////////////////////////////////
	                   ...
	                   if(command_group==0xff)
                        ...
                        SetOneByteData(m_dataflag); 											 
										...
										else if(command_group<=6)
										{
											  SetOneByteData(m_dataflag); 
											  SetFourByteData((float)gDCdistribution.pst_userI[command_group]/10);//租户电流
												SetOneByteData(1);//铁塔自定义字段数量
												SetFourByteData((float)gDCdistribution.pst_userEnerqy[command_group]/100);//租户电能
												SetOneByteData(0);//厂家自定义字段数量  
										}
										else
										{
			...
			if(command_group==0xff)
			{
					...
					for(u8 i=0;i<8;i++)
					{
							...
							SetOneByteData(6);//电池温度数N（N=0）
								for(u8 j=0;j<6;j++)
								{
									 SetFourByteData((float)batt[i].CellTemp[j]/1000);//电池组电压V
								}
...
void CYDT1363::set4180(void)//模块防盗使能
{
              						...
              						if(gbattGroud>0)//电池配电分路存在时
													{
															 ...
															 SetOneByteData(gbattGroud);//电池组数量  电池回路开关数量 m														
															 for(u8 i=0;i<gbattGroud;i++)	 
															 SetOneByteData(fbatt[i]);//电池开关回路状态
...
//																 SetOneByteData(GetWarnInPut(BATTFUSR_BREAK,0x03,1));//电池2熔丝
//														     SetOneByteData(battbroadfuse[0]);//下电板熔丝1
//																 SetOneByteData(battbroadfuse[1]);//下电板熔丝2
														
														   
													}		
											           SetOneByteData(0);//直流负载回路数
																 SetOneByteData(0);//绝缘告警（未用项）										       
																 //SetOneByteData(2+battGroud);									//铁塔自定义字段数量 a=1+1+m m=1
													       SetOneByteData(2+1);	
													       SetOneByteData(fbatt[6]);//电池下电
																 //SetOneByteData(battGroud);//电池组数量		
                                // for(u8 i=0;i<battGroud;i++)		
...
void CYDT1363::set4246(void)//直流瓶获取参数
{
	      ...
	      SetFourByteData((float)goutPutOwrV/100);//电压告警下限
			...
			// SetOneByteData(21+6+battGroud);//自定义P=21+6+m
				...
				if(gbattGroud == 0)
				{   u8  tbattGroud = 1;
							SetOneByteData(21+1+tbattGroud);//自定义P=21+1+m
							SetFourByteData(tbattGroud);//电池组数量 m
						//	for(u8 i=0;i<battGroud;i++)
							SetFourByteData(gbranchBatt[2]);//电池容量
				}
	...
	u8 command_group=0;
	        ...
	        if(AsciitoHex(&command_group,&m_recvdata[0])==0){ReturnVailData(6);return ;}//无效命令;
	         if(command_group==0xff)
					 {						 
							...
							for(u8 i=0;i<6;i++)
						{
							 ...
							 if((gSwitchPara[TOTAL_USER+1+i].st_downMode&0x0F)>0)
							 {
								...
								SetFourByteData(gSwitchPara[TOTAL_USER+1+i].st_downT);//直流配电单元一次下电时间
								SetFourByteData((float)gSwitchPara[TOTAL_USER+1+i].st_downV/100);//直流配电单元一次下电电压
					...
					{
						   u8 tmpdata=0;
							 if((gSwitchPara[TOTAL_USER+command_group].st_downMode&0x07)>0)
							 {
								 tmpdata=1;		 
							 }
								SetFourByteData(tmpdata);//直流配电单元一次下电使能
								SetFourByteData(gSwitchPara[TOTAL_USER+command_group].st_downT);//直流配电单元一次下电时间
								SetFourByteData((float)gSwitchPara[TOTAL_USER+command_group].st_downV/100);//直流配电单元一次下电电压
								SetOneByteData(1);//铁塔自定义字段数量 a=1 1
	   ...
	   u8 branchCount=0,SearchCount=0;
		 ...
		 if(command_group!=0xff){
		  ...
		  if(sysPara[PROL_OLD_FLAG] == 1)
		  {
			...
			else if(sysPara[PROL_OLD_FLAG] == 0)
			{
...
void CYDT1363::set42A9(void)//获取削峰参数
{
                   ...
                   SetOneByteData(gPeakShaveOnoff);
                         SetOneByteData(0);
                    SetFourByteData((float)gSinglePhasePower/10);
                    ...
                    if((cmd_type>=0x10)&&(cmd_type<=0x13))//错峰系数 0x10~0x13
										...
										else if(cmd_type==0x18)//电池充电模式
										{
											 gWorkShiftMode=tmpdatu8;
											   if(gWorkShiftMode == 0)//关闭错峰时，打开电池充电mos
												 {
														 u16 setmos=1;
		  ...
		  if(cmd_type==0x10)//削峰开关 
								 ...
								 else if(cmd_type==0x12)//单相模式总功率 
								 {
									    gSinglePhasePower=tmpdatfloat*10;
									        gAPhasePower =  gSinglePhasePower/3;
									        gBPhasePower =  gSinglePhasePower/3;
									        gCPhasePower =  gSinglePhasePower/3;
								 ...
								 else if(cmd_type==0x13)//L1 相功率
								 {
									    gAPhasePower=tmpdatfloat*10;
									     pgh52c0->savePara(&gAPhasePower);
									     preModuleCount=0;
									     gSinglePhasePower = gAPhasePower+gBPhasePower+gCPhasePower;
								 }
...
void CYDT1363::setXX51F(void)//工厂信息
{
	...
	else if(sysPara[SELECT_FACT] == 1)
	{
		 sprintf((char *)factory,(const char *)"Mentech");
	}
  else if(sysPara[SELECT_FACT] == 2)
	{
		 sprintf((char *)factory,(const char *)"CICT");
	}
	else if(sysPara[SELECT_FACT] == 3)
	{
		 sprintf((char *)factory,(const char *)"AWESOME");
	}
