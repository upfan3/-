...
const u8 * battlist[]={(const u8 *)"57V",(const u8 *)"48V",NULL};
...
const u8 * ALarmList[]={(const u8 *)"无",(const u8 *)"输出电压",(const u8 *)"电池SOC"
	                   ,(const u8 *)"A相电压",(const u8 *)"B相电压",(const u8 *)"C相电压"
										 ,(const u8 *)"模块输入电流",(const u8 *)"外设",(const u8 *)"电池单体电压"
										 ,(const u8 *)"交流频率"
										 ,(const u8 *)"缺A相" ,(const u8 *)"缺B相"  ,(const u8 *)"缺C相"    
	                   ,(const u8 *)"分路熔丝",(const u8 *)"分路",(const u8 *)"模块"
...
const u8 * ALarmnbList[]={(const u8 *)" ",(const u8 *)"1号",(const u8 *)"2号",(const u8 *)"3号",(const u8 *)"4号",										 
                          ...
                          (const u8 *)"40号",(const u8 *)"41号",(const u8 *)"42号",(const u8 *)"43号",(const u8 *)"44号",	NULL														
};
...
const u8 *switchFactory[]={(const u8 *)"JG",(const u8 *)"TY",(const u8 *)"XD",NULL};
void   *SwitchFactory[]={switchFactory};


const u8 *jumpprotocol[]={(const u8 *)"物理",(const u8 *)"逻辑",NULL};//跳空， 非跳空
void   *JumpProtocol[]={jumpprotocol};

const u8 *factoryName[]={(const u8 *)"ZTKD",(const u8 *)"Mentech",(const u8 *)"CICT",(const u8 *)"AWESOME",NULL};
void *FactoryName[]={factoryName};
				 ...
				 { 
                     SetScreenItem(pMainMenu,   (u8 *)"设置"      ,&passWord,(u8 *)(&pSetPara),NEXT_SCREEN);
				             ...
				             SetScreenItem(NULL,        (u8 *)"整流模块"  ,NULL,(u8 *)(&pSMRMenu),NEXT_SCREEN);
					           ...
					           SetScreenItem(NULL,        (u8 *)"用户信息"  ,NULL,(u8 *)(&pUSR2Menu),NEXT_SCREEN);
					           SetScreenItem(NULL,        (u8 *)"分路信息"  ,NULL,(u8 *)(&pUSR3Menu),NEXT_SCREEN);
					           //SetScreenItem(NULL,        (u8 *)"电费测算"  ,NULL,(u8 *)(&pPOWERRATEMenu),NEXT_SCREEN);
				...
				else if(ppItem==&pSetPara)//设置界面
			{
				...
				if(pSetPara!=NULL)
				{
				  ...
				  SetScreenItem(pSetPara,   (u8 *)"参数设置" ,NULL,(u8 *)(&pSYSMenu),NEXT_SCREEN);
					...
					SetScreenItem(NULL,       (u8 *)"配电设置 ",NULL,(u8 *)(&pSetBranchMenu),NEXT_SCREEN);
					SetScreenItem(NULL,       (u8 *)"铅酸电池单元设置",NULL,(u8 *)(&pBattMenu),NEXT_SCREEN);//	
			...
			else if(ppItem==&pSysParaSetMenu)//工厂设置界面
			{
			     ...
			     pdisDisPlayData[22]=sysPara[DELAY_VOLT_UP];
				   pdisDisPlayData[23]=sysPara[SELECT_FACT];
					 pdisDisPlayData[24]=sysPara[LIDIAN_MODEL];
			    ...
			    SetScreenItem(pSysParaSetMenu,    (u8 *)"时间设置        ",NULL,(u8 *)(&pTimeMenu),NEXT_SCREEN);	
					...
					SetScreenItem(NULL,               (u8 *)"电压校准      ", NULL,(u8 *)(&pCalibrationVMenu),NEXT_SCREEN);//
						...
						SetScreenItem(NULL,             (u8 *)"锂电%l模式    ",     BattLis   ,&pdisDisPlayData[24]  ,MODIFI_PARA);//
					...
					//SetScreenItem(NULL,               (u8 *)"电池类型      %l",  BattLis,          &pdisDisPlayData[18],MODIFI_PARA);	
				...
				{	
					...
					SetScreenItem(NULL,               (u8 *)"%l"              ,  FactoryName,      &pdisDisPlayData[23],MODIFI_PARA);	
				}
...
//				   u8 *para8=(u8 *)tmpPara;
			 ...
			 else if(ppItem==&pDCDMenu)//直流配电界面（主界面）
		  {
			 ...
			 if(pDCDMenu!=NULL)
			 {
				  ...
				  SetScreenItem(NULL,   (u8 *)"电池电流    %F.1A" ,   &gpSysData[TOTAL_BATTI],0,SCREEN_TWO_BYTE);//BATT_SOC
				 	SetScreenItem(NULL,   (u8 *)"电池SOC     %F.2%",     &gpSysData[BATT_SOC],0,SCREEN_TWO_BYTE);//
				  SetScreenItem(NULL,   (u8 *)"储能电流    %F.1A" ,   &totalBattI,0,SCREEN_TWO_BYTE);//BATT_SOC
//				
				  SetScreenItem(NULL,   (u8 *)"储能SOC     %F.2%",     &totalSOC,0,SCREEN_FOUR_BYTE);//

  			  SetScreenItem(NULL,   (u8 *)"电池温度    %F.2℃",		  &gpSysData[ENV_TEMP],0,SCREEN_TWO_BYTE);//;
		  ...
		  {    
			 ...
			 else if(ppItem==&pUSR3Menu)//用户单元界面
			{
				   ...
				   if(pUSR3Menu!=NULL)
			 {
				    ...
				    SetScreenItem(pUSR3Menu,   (u8 *)"用户分路      %O/$D",             &pdisDisPlayData[2],0,MODIFI_PARA);
				    SetScreenItem(NULL,        (u8 *)"电流  %F.1A",     &pdisDisPlayData[4],0,SCREEN_TWO_BYTE); 	
						SetScreenItem(NULL,        (u8 *)"电量  %G.1KWH",   &pdisDisPlayData[6],0,SCREEN_FOUR_BYTE); 
					  SetScreenItem(NULL,        (u8 *)"百分比 %F.1%",       &pdisDisPlayData[0],0,SCREEN_TWO_BYTE); 

				 	SetScreenItem(NULL);
				 SetScreenItemSelectFirst(pUSR3Menu);
				 		 return 1;
			 }
			  else 
				 return 0;
	...
	*/		
         ...
         para16[4]=gSetBattLimitPerC;
         	...
         	SetScreenItem(NULL,           (u8 *)"温补系数      %dmV/℃",& para16[2],(u8*)&gtmpFactor16,MODIFI_PARA|SCREEN_TWO_BYTE);
					...
					{
							SetScreenItem(NULL,            (u8 *)"铅酸单元组  %D"   ,&para8[2] , &battGroud,MODIFI_PARA);					
							if(battGroud>0)
							{
	...
	/*		
          ...
          (*(u16 *)(&pdisDisPlayData[13]))=recoverV1;
				...
				if(pUsrDownParaSetMenu!=NULL)
				{
					...
					SetScreenItem(pUsrDownParaSetMenu ,   (u8 *)"电池分路    %D"      , &pdisDisPlayData[0],0,MODIFI_PARA);
...
//							pdisDisPlayData[6]=gsleepBattTestTime;
          ...
          SetScreenItem(NULL);//结束
			...
			else if(ppItem==&pPeakShiftMenu)//错峰界面
			{
				...
				*((u16 *)(&pdisDisPlayData[9]))=gBattDisVolt;
				*((u16 *)(&pdisDisPlayData[11]))=gBattChgVolt;
          ...
          SetScreenItem(NULL,   (u8 *)"电池放电深度  %F.2%"     , &pdisDisPlayData[3],(u8*)&gdisDeep,MODIFI_PARA|SCREEN_TWO_BYTE);
					...
					SetScreenItem(NULL,   (u8 *)"锂电放电电压  %F.2V"     , &pdisDisPlayData[9],(u8*)&gBattDisVolt,MODIFI_PARA|SCREEN_TWO_BYTE);
					SetScreenItem(NULL,   (u8 *)"锂电充电电压  %F.2V"     , &pdisDisPlayData[11],(u8*)&gBattChgVolt,MODIFI_PARA|SCREEN_TWO_BYTE);
          ...
          SetScreenItem(NULL);//结束
			...
			else if(ppItem==&pPeakShaveMenu)//削峰界面
			{
				...
				if(pPeakShaveMenu!=NULL)
				{
		                ...
		                SetScreenItem(NULL,   (u8 *)"总功率  %F.1Kw"    ,&pdisDisPlayData[1],(u8*)&gSinglePhasePower ,MODIFI_PARA|SCREEN_TWO_BYTE);//尖峰时段交流限功率值
...
//					 SetScreenItem(NULL,              (u8 *)"温度RevC %F.2uA",  (u8 *)&tempI  ,          NULL,                SCREEN_TWO_BYTE);	
         ...
         {pdisDisPlayData[i]=0;}
				   ...
				   if(pFactorySettingMenu!=NULL)
					 {
						  ...
						  SetScreenItem(NULL,                 (u8 *)"恢复开关参数     %l", OnOffLis, &pdisDisPlayData[6],MODIFI_PARA);
				   ...
				   if(pSetBranchParaMenu!=NULL)
					 {
						 ...
						 SetScreenItem(pSetBranchParaMenu,    (u8 *)"分路:%O/$O  槽位:$O   "  , &pdisDisPlayData[0],NULL,MODIFI_PARA);
                            ...
                            SetScreenItem(NULL,   (u8 *)"授权        %l"   ,     &OnOffLis   ,&pdisDisPlayData[11],setSpecialParaFlag);						 
													 ...
													 {
															SetScreenItem(NULL ,   (u8 *)"过流恢复上电   %l"     ,OnOffLis,& pdisDisPlayData[13],MODIFI_PARA);
														}
			...
			else if(ppItem==&pBattMenu)//铅酸单元设置
			{
				  ...
				  SetScreenItem(pBattMenu,     (u8 *)"电池电流校准 ", NULL,(u8 *)(&pCalibrationMenu),NEXT_SCREEN);
				  SetScreenItem(NULL,          (u8 *)"电池单元校准 ", NULL,(u8 *)(&pCalibrationMenu2),NEXT_SCREEN);
			...
			else if(ppItem==&pCalibrationMenu2)//铅酸单元下电板校准
			{
				 ...
				 SetScreenItem(pCalibrationMenu2, (u8 *)" %l"          ,BattList1             ,& pdisDisPlayData[0],MODIFI_PARA);
				 ...
				 SetScreenItem(NULL,              (u8 *)"启用    %l"    , OnOffLis,            &pdisDisPlayData[6],  MODIFI_PARA);
				 SetScreenItem(NULL);//结束
					return 1;
			}
			else if(ppItem==&pBatterySetMenu)//锂电设置界面
			{
				...
				(*(u16 *)&pdisDisPlayData[2])=batt[0].Set_BattA;
	...
	//				SetScreenItem(NULL,(u8 *)"放电电流      %F.2A",&pdisDisPlayData[2],NULL,MODIFI_PARA|SCREEN_TWO_BYTE);
					SetScreenItem(NULL,(u8 *)"充电电流      %F.2A",&pdisDisPlayData[2],NULL,MODIFI_PARA|SCREEN_TWO_BYTE);
	   ...
	   Item ** CurrentPpItem=pCurrentScreen->m_layer[pCurrentScreen->m_LayerCount].st_ppItem;
	             ...
	             if(CurrentPpItem==&pDCDMenu)//直流配电单元界面处理
							...
							*/
							...
							{
								  SetCalibrationMenu2(pCurrentScreen);
							}
