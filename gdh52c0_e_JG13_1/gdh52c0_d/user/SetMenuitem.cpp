	 ...
	 u16 *precoverV=&recoverV1;
	        ...
	        if(pdisDisPlayData[1]>4) pdisDisPlayData[1]=0;
					...
					else if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_plink==&pdisDisPlayData[13])
				{
					             ...
					             CheckParaBoundary((u16 *)&pdisDisPlayData[13],gDcParam0[pdisDisPlayData[0]-1].st_LDVoltage+50,5400);
					              
					                    if( genterflag==1)
															{        
					                                precoverV[pdisDisPlayData[0]-1]=(*(u16 *)(&pdisDisPlayData[13]));
					                             pgh52c0->savePara(&precoverV[pdisDisPlayData[0]-1]);
           ...
           if((pCurrentScreen->m_EnterStatus!=3)||//在非设置参数时可赋值
				...
				{
					              if(pdisDisPlayData[0]>=3) pdisDisPlayData[0]=1;
					              if(pdisDisPlayData[0]==0) pdisDisPlayData[0]=2;
										 ...
										 (*(u16 *)(&pdisDisPlayData[13]))=precoverV[pdisDisPlayData[0]-1];
...
void SetPeakShiftPara(Screen *pCurrentScreen)//错峰设置
{
	   if(((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[0]) &&(genterflag==1))
							    ...
							    else	if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==(u8*)&gBattRecovSoc) 
							 {
							  ...
							  else	if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==(u8*)&gBattDisVolt) 
							 {
								                 ...
								                 if(genterflag==1)
																 {
																   ...
																   pgh52c0->savePara(&gBattDisVolt);
																 }
							 } 
							 else	if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==(u8*)&gBattChgVolt) 
							 {
      ...
      if(((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==pdisDisPlayData)&&(genterflag==1)) //光标在“清除行"，可进行清除操作
				{
		          ...
		          { genterflag=0;
								    if(pdisDisPlayData[0]==1)
										{
											 pdisDisPlayData[0]=0;
											if(alarmdisplay[0]==MODULE)
											{
											 removeModule(alarmdisplay[1],MODULE);
											}
											else if((alarmdisplay[0]==USER_DOWN)&&(alarmdisplay[2]==COMM_BROKEN))
											{
												removeModule(alarmdisplay[1],USER_DOWN);
											}
										}
...
void SetOter(Screen *pCurrentScreen)//工厂设置
{
		 ...
		 else if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[5])
    {
		...
		else  if(((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[18])&&(genterflag==1))
		{
								 ...
								 gbattType=pdisDisPlayData[18];
								 pgh52c0->savePara(&gbattType);
		...
		else  if(((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[24])&&(genterflag==1))
		{
			         
			
			 sysPara[LIDIAN_MODEL]=pdisDisPlayData[24];
			
			   if(sysPara[LIDIAN_MODEL] == 0)
				 {
					 
					   gsetbatt.Set_BattV=5700; 
					   SetBatteryData(1,0,(u8 *)&gsetbatt.Set_BattV);
					   
				 }
				 else  if(sysPara[LIDIAN_MODEL] == 1)
				 {
					 
					   gsetbatt.Set_BattV=4800;
					   SetBatteryData(1,0,(u8 *)&gsetbatt.Set_BattV);
				 }
						    ...
						    if(preAlarmCount!=getaddr)
							   ...
							   pgh52c0->readI2C(ALARM_DATA_STAR+6+12*getaddr,(u8 *)&tmpalarmRecord,2);//非第一次记录，读取存储记录数
							    ...
							    preAlarmCount=getaddr;
								     ...
								     if(alarmRecordType[0]>44)  alarmRecordType[1]=0;
	 ...
	 u8 usegroud[12]={0,1,1,2,2,3,3,4,4,6,5,7};//0无, 1移动 ，2联通，3电信，4广电，6铁塔，5行业外，7电池
       ...
       if(((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[3])&&(genterflag==1))//下电模式
			 ...
			 else if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[11])//截止日期
			 {
				...
				else if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_pParalist==&pdisDisPlayData[13])//过流恢复上电
			 {
				 if(gSwitchData[ branchCount].authorizeflag== 0)
				  {
					  pdisDisPlayData[13]=0;
					  gSwitchData[ branchCount].recpowerFlag=0;
				  }
					  
				 if(genterflag==1)
				 {
					genterflag=0;
					  if(pdisDisPlayData[13]==1 &&gSwitchData[ branchCount].authorizeflag==1 )
					  {
								gSwitchData[ branchCount].recpowerFlag=1;
					  }
...
void SetUSR3Menu(Screen *pCurrentScreen)
{
	     ...
	     if (pdisDisPlayData[2]<1) pdisDisPlayData[2]=2;//通道号
	     if (pdisDisPlayData[2]>2) pdisDisPlayData[2]=1;
	
	         if(pdisDisPlayData[2]==1)
	   		 	  *((u16 *)&pdisDisPlayData[4])=*((s16 *)&gpSysData[BATT_CURR1]);
					 else if(pdisDisPlayData[2]==2)
						*((u16 *)&pdisDisPlayData[4])=*((s16 *)&gpSysData[BATT_CURR2]);
...
void SetCalibrationMenu2(Screen *pCurrentScreen)
{
	    ...
	    if(pdisDisPlayData[0]==0)
			{
		   *(s16 *)&pdisDisPlayData[2] = *(s16 *)&gpSysData[BATT_CURR5];
				       
			}
			else if(pdisDisPlayData[0]==1)
			{
			 *(s16 *)&pdisDisPlayData[2] = *(s16 *)&gpSysData[BATT_CURR6];  
				      
			}
	    else if(pdisDisPlayData[0]==2)
			{
	     *(s16 *)&pdisDisPlayData[2] = (*(s16 *)&gpSysData[ACID_BATT_VOL])/10;    
	    ...
	    if((pCurrentScreen->m_pItem[pCurrentScreen->m_head+pCurrentScreen->m_coursItem]).st_plink==&pdisDisPlayData[4])//电量设置
			...
			if(pCurrentScreen->m_EnterStatus!=3)
			{
				   if(pdisDisPlayData[0]==0)
						{					  
											pdisDisPlayData[6] = sysPara[BATT5_EN];
						}
						else if(pdisDisPlayData[0]==1)
						{			
											pdisDisPlayData[6] = sysPara[BATT6_EN];
						}
						else if(pdisDisPlayData[0]==2)
						{			
											pdisDisPlayData[6] = 0;
